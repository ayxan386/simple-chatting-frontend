...
export default function Home() {
  ...
  const [startSession, openSession] = useState(false);
      ...
      </Head>
      {
        ...
        startSession ?
          <div className={styles.pop_up_window + " row"}>
            <div className={'col-5 ' + styles.pop_up_box}>Hello world</div>
            <button onClick={e => openSession(false)}>Close</button>
          </div>
          : <></>
      }
      <main className={`${styles.main} container `}>
        ...
        <div className={`${styles.main_wrapper} row`}>
          <div className={`col-6 ${styles.main_contact_holder}`}>
            ...
            {
              contacts
                .map((contact: ContactDto) => ContactListItem(contact, openSession))
            }
          </div>
